...
# Note: Dataset-specific training modules moved to model_zoo/[dataset]/[dataset].py
# Use run_train_unified.py for unified training across datasets
...
def run_multiprocess(rank, world_size, cfg, port, dataset_name=None, architecture=None):
    try:
        setup(rank, world_size, port)
        _run(rank, world_size, cfg, dataset_name, architecture)
    finally:
        cleanup()


def _run(rank, world_size, cfg, dataset_name=None, architecture=None):
    # print (rank)
    ...
    # Build data iterators - legacy method for compatibility
...
# Main function for compatibility with new factory-based training
def main(rank, cfg, dataset_name=None, architecture=None):
    """Main entry point for training with optional dataset and architecture specification."""
    world_size = cfg.ngpus
    
    if world_size > 1:
        # For multi-GPU, this should be called from multiprocessing
        _run(rank, world_size, cfg, dataset_name, architecture)
    else:
        # Single GPU training
        _run(0, 1, cfg, dataset_name, architecture)

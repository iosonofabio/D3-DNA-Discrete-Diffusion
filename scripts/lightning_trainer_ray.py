from h5py._hl.dataset import numpy
import torch
...
class D3DataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: str = None):
        ...
        if self.cfg.training.data_fraction < 1.0:
            n_samples_train = len(self.train_ds)
            n_samples_val = len(self.val_ds)
            indices_train = random.sample(range(n_samples_train), int(n_samples_train*self.cfg.training.data_fraction))
            indices_val = random.sample(range(n_samples_val), int(n_samples_val*self.cfg.training.data_fraction))
            self.train_ds = Subset(self.train_ds, indices_train)
            self.val_ds = Subset(self.val_ds, indices_val)

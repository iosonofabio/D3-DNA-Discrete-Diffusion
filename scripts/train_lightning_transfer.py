...
def main():
    ...
    parser.add_argument('--transfer', type=bool, default=None,
                       help='Modify first/last layers to transfer learn on new data')
    ...
    if args.fraction_data < 1.0:
        print(f"Using {int(args.fraction_data*100)}% of training data")
    print(f"Config: {cfg}")
    ...
    # load weights and (possibly) modify first/last layers to transfer learn on new data
    if args.transfer:
        # load weights
    ...
    print(f"Working directory: {work_dir}")
    if args.fraction_data < 1.0:
        print(f"Data fraction: {int(args.fraction_data*100)}%")
    if args.resume_from:

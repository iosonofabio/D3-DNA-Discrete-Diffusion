...
def load_trained_model(checkpoint_path: str, config: DictConfig, architecture: str, device: str = 'cuda') -> Tuple[torch.nn.Module, Any, Any]:
    """
    Load trained DeepSTARR model from specific checkpoint file.
    ...
    if architecture.lower() == 'transformer':
        ...
        model = DeepSTARRConvolutionalModel(config)
    else:
    ...
    print(f"✓ DeepSTARR {architecture} model loaded successfully")
